...
static void doI2Cdetect (UNU int argc, char *argv [])
{
  ...
  sprintf (command, "%s -y -r %d", c, port) ;
  if (system (command) < 0)
...
static void doPadDrive (int argc, char *argv [])
{
  ...
  if ((pin < 0) || (pin > 120))
  {
    fprintf (stderr, "%s: drive pin not 0-120: %d\n", argv [0], pin) ;
    exit (1) ;
  }

  if ((val < 0) || (val > 15))
  {
    fprintf (stderr, "%s: drive value not 0-15: %d\n", argv [0], val) ;
    exit (1) ;
  }
...
static void doPwmFreq (int argc, char *argv [])
{
  int pin;
  ...
  if (argc != 4)
  {
    fprintf (stderr, "Usage: %s pwmf pin <frequency>\n", argv [0]) ;
    exit (1) ;
  }

  pin = atoi (argv [2]) ;
  frequency = (unsigned int)strtoul (argv [3], NULL, 10) ;

  if (frequency == 0)
  {
    fprintf (stderr, "%s: frequency must be > 0\n", argv [0]) ;
    exit (1) ;
  }

  pwmSetFreq (pin, frequency) ;
}

static void doPwmDuty (int argc, char *argv [])
{
  int pin;
  ...
  if (argc != 4)
  {
    fprintf (stderr, "Usage: %s pwmd pin <duty_cycle>\n", argv [0]) ;
    exit (1) ;
  }

  pin = atoi (argv [2]) ;
  duty_cycle = (unsigned int)strtoul (argv [3], NULL, 10) ;

  if (duty_cycle == 0)
  {
    fprintf (stderr, "%s: duty_cycle must be > 0\n", argv [0]) ;
    exit (1) ;
  }

  pwmSetDuty (pin, duty_cycle) ;
}
...
// Pi Specifics
  ...
  else if (strcasecmp (argv [1], "pwmf"     ) == 0) doPwmFreq    (argc, argv) ;
  else if (strcasecmp (argv [1], "pwmd"     ) == 0) doPwmDuty    (argc, argv) ;
